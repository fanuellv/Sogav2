...
@section('content')
    ...
    <div id="modal-bg" class="modal-bg" onclick="closeModal(event)">
        <div class="publicacion-modal" onclick="event.stopPropagation();">
            <form action="{{ route('CriarPostagem') }}" method="post" class="forma" enctype="multipart/form-data">
                @csrf
    ...
    <div class="post">
        @foreach ($posts as $posts)
            <div class="postItem">
                <div class="topPost">
                  @if ($posts->user)
                        <div class="fotoPost" style="background-image: url('{{ asset($posts->user->fotoPerfil) }}');"></div>
